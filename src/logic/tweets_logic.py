...
class TweetOperations:
    ...
    async def tweet_post(self, tweet_schema: TweetCreateSchema):
        tweet_id: int = await model_manager.create_tweet(tweet_schema.tweet_data, self.user_id)
    ...
    async def get_all_tweets(self):
        ...
        for tweet in tweets:
            ...
            tweet_schema = TweetSchema(id=tweet.id,
                                       content=tweet.content,
                                       attachments=[img.file_path for img in tweet.image]
                                       if tweet.image is not None else [],
                                       author=author,
                                       likes=[LikeSchema(user_id=like_user.id, name=like_user.name)
                                              for like_user in tweet.like_by_user]
                                       if tweet.like_by_user is not None else [])
