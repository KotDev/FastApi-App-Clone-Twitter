...
from database.schemas import UserCreateSchema, UserInfoSchema, SuccessfulResponseSchema
...
async def register_user_router(operation: UserOperations = Depends(get_user_operations),
...
async def get_user_me_router(request: Request,
...
@router.get("/{user_id}")
@check_user_authorization()
async def get_user_router(user_id: int,
                          operation: UserOperations = Depends(get_user_operations)):
...
@check_user_authorization()
async def follow_user_router(follower_id: int,
                             request: Request,
                             operation: UserOperations = Depends(get_user_operations)):
    api_key: str = request.headers.get("api-key")
    user_id: int = await model_manager.get_user(api_key).id
...
async def follow_user_router(follower_id: int,
    ...
    api_key: str = request.headers.get("api-key")
    user_id: int = await model_manager.get_user(api_key).id
    result: SuccessfulResponseSchema = await operation.delete_follow(user_id, follower_id)
    return result
