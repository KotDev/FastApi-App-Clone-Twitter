...
async def create_tweet_router(
    schema_create: TweetCreateSchema,
    request: Request,
...
@router.get("")
@check_user_authorization
async def get_all_tweets_router(
...
@check_user_authorization
async def add_like_tweet_router(
    tweet_id: int,
    request: Request,
    operation: TweetOperations = Depends(get_operation_tweets),
):
...
@check_user_authorization
async def delete_like_tweet_router(
    tweet_id: int,
    request: Request,
    operation: TweetOperations = Depends(get_operation_tweets),
):
    result: SuccessfulResponseSchema = await operation.like_tweets_delete(
        tweet_id=tweet_id
    )
    return result
